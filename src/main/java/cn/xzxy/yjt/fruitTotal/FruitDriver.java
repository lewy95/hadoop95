...
public class FruitDriver {
    ...
    public static void main(String[] args) throws IOException,
        ...
        job.setMapOutputValueClass(Fruit.class);
        ...
        job.setOutputValueClass(Fruit.class);

        //5.1设置处理文件的hdfs的输入路径(导包导长的)，路径写到目录级别
        FileInputFormat.setInputPaths(job,
                new Path("hdfs://192.168.80.128:9000/mr/fruit"));
        //5.2设置结果输出路径,输出目录不能存在
        FileOutputFormat.setOutputPath(job,
                new Path("hdfs://192.168.80.128:9000/mr/fruit/result"));
